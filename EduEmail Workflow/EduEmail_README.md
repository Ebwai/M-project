...
2. 启动 n8n，例如： docker run -it --rm --name n8n -p 5678:5678 -v n8n_data:/home/node/.n8n --restart always docker.n8n.io/n8nio/n8n
...
1. 配置 Microsoft Outlook OAuth2 凭证，用于读取邮件 + 写入日历。

2. 配置 AI 模型 API Key（如 OpenAI / 阿里云千问）。

3. 在打分与路由 Code 节点里，替换为你自己邮箱的真实 Folder ID。

## ✅ 已完成特性

- HTML 邮件转 Markdown 降噪

- 附件图片识别 + 统一命名 + 交给视觉大模型

- 邮件正文与海报内容合并处理

- 分层打分引擎（私人/机构、考试、求职、出国、补贴等维度）
